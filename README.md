...
You bring the embedding function and the LLM. We handle chunk, embed, store, and retrieve.
...
const embed = async (texts: string[]) => {
  const res = await fetch('http://localhost:8000/v1/embeddings', {
    ...
    body: JSON.stringify({ input: texts, model: 'my-model' })
  })
...
| Method | Description |
|--------|-------------|
| `new Rag({ connectionString, dimension? })` | Create instance (default dim: 2048, lazy init) |
