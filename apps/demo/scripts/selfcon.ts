...
import { createRerankingModel } from './utils'
...
const tallyVerdicts = (verdicts: (string | undefined)[]): Map<string, number> => {
  ...
  main = async () => {
    ...
    const parsed = parseArgs(args),
      ...
      outputPath = typeof outputFlag === 'string' ? outputFlag : 'out-selfcon.json',
      limitFlag = parsed.get('--limit'),
      limit = typeof limitFlag === 'string' ? Number.parseInt(limitFlag, 10) : 50,
      ...
      rerankTop = typeof rerankTopFlag === 'string' ? Number.parseInt(rerankTopFlag, 10) : 30,
      runsFlag = parsed.get('--runs'),
      runs = typeof runsFlag === 'string' ? Number.parseInt(runsFlag, 10) : 5,
      tempFlag = parsed.get('--temperature'),
      temperature = typeof tempFlag === 'string' ? Number.parseFloat(tempFlag) : 0.3,
